...
<!-- TODO(screenshots): docs/screenshots/*.jpg は周期 10 分・3 波だった以前の設定で撮影したもの（最初の浸水 3:54、浸水範囲 約 3.5 km² など）。
     周期 20 分・6 波の現在の設定で `npm run screenshots` を実行して撮り直し、このコメントを消すこと。 -->
...
スクリーンショットは相模トラフ沿いの海溝型地震（西側モデル、震度7）を解像度「標準」で計算したものです（`npm run screenshots` で撮り直せます。撮影用に計算時間を短くしている場合があります）。
...
| **ハザードマップ** | 神奈川県の **公式の津波浸水想定**（平成27年、ハザードマップポータルサイトの配信タイル）を重ねて表示できます（不透明度も調整可）。計算結果からは「最大浸水深」と「津波到達時間」（各地点が最初に 1 cm 以上浸水した時刻）の地図を作り、浸水深は公式と同じ配色（国土交通省「水害ハザードマップ作成の手引き」）で比べられます。避難場所（国土地理院 指定緊急避難場所データ・津波。藤沢市が独自に指定する津波避難ビルの多くは含みません）を地図に表示し、「ご利用にあたって」と「情報」タブから藤沢市の津波ハザードマップへ案内します。 | `src/map2d/`、`src/data/sources.ts`、`src/ui/panels/layers.ts` |
| **水位のシミュレーション** | 非線形長波（浅水）方程式を TUNAMI-N2 方式の差分法で解き、陸への遡上（浸水）も計算します。沖合の境界から入れる波の大きさは、海岸の最大水位が公的な想定値になるよう自動で調整し、最大の波がシナリオの到達時間（公的資料の最大津波到達時間など）に来るよう時刻を合わせます。計算は Web Worker（大きな格子は複数ワーカーで並列）で行い、結果を逐次受け取りながら再生します。タイムライン・再生速度・鵠沼海岸沖の水位グラフ・警報の表示があります。 | `src/sim/`、`src/ui/timeline.ts`・`hud.ts` |
...
5. 「人物」タブで人物を置き、避難の見通しと浸水の深さを確かめます（Esc で配置を終了）。
   地図の右上の虫めがねのボタン（地名・住所で探す）で場所を検索したり、現在地のボタンで今いる場所を表示したりして、その場所に人物を置くこともできます。
...
- **指定緊急避難場所データ（津波）**: 市町村が登録した情報で、最新でない場合や掲載されていない場合があります（国土地理院の「ご利用上の注意」）。
...
- ブラウザから外部へ送られる情報: 地名・住所の検索では入力した文字を国土地理院の地名検索API（`msearch.gsi.go.jp`）へ、
  ...
  現在地（ブラウザの位置情報）の座標はどこにも送らず、現在地に置いた人物の住所も調べません。ただし、現在地の周辺を地図に表示すると
  （現在地を取得したときに地図が現在地へ移動する場合を含む）、その範囲の地図画像を配信元（国土地理院など）から読み込むため、
  おおよその場所（数百 m 程度）は配信元に伝わります（地図を動かして表示したときと同じです）。
