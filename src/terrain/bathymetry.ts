 ...
 *
 * 【仮定の限界】距離は「最も近い陸」から測るので、江の島の周囲にも同じ砂浜の断面を与えている。
 * 江の島の南〜西側は海食崖・岩礁（隆起海食台）で砂浜の平衡断面は当てはまらず、実際には島のすぐ近くでも
 * より深い可能性がある（釣り場の案内では島の南側「裏磯」は足元から水深 約10 m:
 * http://www.fishing-spots.net/ura-iso.html 。測量データではない）。定量的な資料が得られないため補正していない。
 * そのため範囲南西部の沖（江の島から 1〜3 km）の推定水深は、本土の汀線からの距離だけで決めた場合より浅くなる。
 */
