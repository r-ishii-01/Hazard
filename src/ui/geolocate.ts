 ...
 *   ただし地図を現在地へ移すので、その周辺の地図画像の読み込みから、おおよその場所は配信元に伝わる（GEO_TILE_NOTE）。
 * - エラーは日本語の説明に置き換える。
 *
 ...
 * タイルの位置から、おおよその場所（数百 m 程度）は配信元に伝わる。現在地を取得すると地図が現在地へ移動する
 * （mapTools.ts）ので、「外部に何も伝わらない」とは書かず、このことを添える（GEO_PRIVACY_TEXT と一緒に出す）。
 */
/** GEO_TILE_NOTE の短い形（人物タブの「現在地に置く」の下など） */
export const GEO_TILE_NOTE_SHORT = '地図を現在地へ移すため、その周辺の地図画像の読み込みで、おおよその場所は地図の配信元に伝わります。';

export const GEO_TILE_NOTE =
  'ただし、現在地を取得すると地図が現在地へ移動し、その周辺の地図画像を国土地理院などの配信元から読み込むため、おおよその場所（数百m程度）は配信元に伝わります（地図を動かして表示したときと同じです）。';
