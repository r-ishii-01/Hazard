 ...
 *   - 表「津波予報の発表条件」（発表される場合・内容の2行）:
 *       「0.2ｍ未満の海面変動が予想されたとき」→ 本サイトの津波予報（若干の海面変動）の区分（forecast.action）
 *       「津波注意報解除後も海面変動が継続するとき」→ 注意報の後に出る別の場合。forecast.note に分けて持つ
...
export interface WarningInfo {
  ...
  announced: string;
  /** 想定される被害（気象庁の表に被害の記載が無い区分は空） */
  damage: string;
  /**
   * とるべき行動。津波予報（若干の海面変動）は、気象庁の表「津波予報の発表条件」の
   * 「0.2ｍ未満の海面変動が予想されたとき」の内容（原文どおり。「…旨を発表します。」で終わる）。
   */
  action: string;
  /** 同じ種類の情報が別の場合にも発表されるときの、その場合と内容（原文どおり。この区分の説明には当てはまらない参考） */
  note?: WarningNote;
  color: string;
  textColor: string;
}

/** 気象庁の表の別の行（同じ種類の情報が別の場合にも発表されるとき） */
export interface WarningNote {
  /** 発表される場合（例: 「津波注意報解除後も海面変動が継続するとき」） */
  when: string;
  /** その場合の内容 */
  text: string;
}
...
export const WARNING_INFO: Record<WarningLevel, WarningInfo> = {
  ...
  forecast: {
    ...
    // 表「津波予報の発表条件」には「想定される被害」の欄が無い（被害の心配がないことは action の文に含まれる）
    damage: '',
    // 表「津波予報の発表条件」の「0.2ｍ未満の海面変動が予想されたとき」の内容（原文どおり）
    action: '高いところでも0.2m未満の海面変動のため被害の心配はなく、特段の防災対応の必要がない旨を発表します。',
    // 同じ表の「津波注意報解除後も海面変動が継続するとき」の内容（原文どおり）。注意報が出ていない本サイトの例には当てはまらない
    note: {
