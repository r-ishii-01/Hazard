...
export const SCENARIO_NOTES: string[] = [
  ...
  '到達時間は原則として公的資料の「最大津波到達時間」（最大の波が来る時刻）です（南海トラフは内閣府の津波高+3m到達時刻、遠地津波の例は短縮した値。各シナリオの説明を参照）。実際にはそれより前に小さな波が来ることがあります。このモデルでは第1波を最大とし、後の波は小さくしていますが、実際には後の波が高いこともあり、津波は数時間以上くり返し来ます。',
  '周期・波の数・揺れの長さ、および一部のシナリオの第1波の向き（押し波か引き波か）は、公的資料に値が無いための設定値（仮定）です。',
];
...
/*
 * 周期の仮定について: 本サイトの入射波は正弦波で、山（押し波）の継続は周期の半分。
 * [K-解説] p.22–23 の計算波形（藤沢市＝湘南港・西側、茅ヶ崎市・中央）では、最初の大きな高まりは数分〜約10分続き、
 * その後の山の間隔は数分〜20分程度とばらつく。公的資料に「周期」の値は無いため、10分は設定値（仮定）。
 */
const ASSUME_SAGAMI_PERIOD =
  '周期10分（仮定。公的資料に周期の値は無い。県の計算波形では最初の大きな高まりが数分〜約10分続き、その後の波の間隔は数分〜20分程度とばらつく）';
const ASSUME_FAR_PERIOD = '周期20分（仮定。公的資料に周期の値は無い）';
...
export const SCENARIOS: ScenarioInfo[] = [
  // ---- 神奈川県 津波浸水想定（平成27年）: 藤沢市の最大クラス ------------------------
  {
    ...
    assumptions: [ASSUME_SAGAMI_PERIOD, ASSUME_WAVES, '揺れ120秒（演出用の仮定）'],
    refs: [
    ...
    description:
      ...
      '8分に最大の波が来るよう設定しています。周期10分・3波・揺れ120秒は設定値（仮定）です。',
  },
  {
    id: 'sagami-central',
  ...
  },
  {
    ...
    assumptions: [ASSUME_SAGAMI_PERIOD, ASSUME_WAVES, '揺れ100秒（演出用の仮定）', '第1波は押し波から（仮定）'],
    refs: [
    ...
    description:
      ...
      '周期10分・3波・押し波から・揺れ100秒は設定値（仮定）です。',
  },
  {
    id: 'taisho',
    ...
    firstMotion: 'rise',
    waves: 3, // 仮定（鎌倉で「2・3回おしよせたり」: [萬年2013] §4.4）
    ...
    assumptions: [
      ...
      // [萬年2013] §4.4 が紹介する梶浦（1986）
      '1923年の津波について、鎌倉付近の計算上の周期は20分程度とする見解もある（萬年ほか 2013 が紹介）',
      '有意な波3波（仮定。1923年の鎌倉では「2・3回おしよせた」記録）',
      '揺れ90秒（演出用の仮定）',
    ],
    refs: [
    ...
    description:
      ...
      '第1波は押し波からとしました（鵠沼の証言に合わせた設定。鎌倉・逗子では最初に少し引いた記録もあります）。周期10分・3波・揺れ90秒は設定値（仮定）です。',
  },
  {
    id: 'keicho',
    name: '慶長型地震',
    ...
    assumptions: ['震度5強（仮置き。公的な震度推計なし）', ASSUME_FAR_PERIOD, ASSUME_WAVES, '揺れ90秒（演出用の仮定）', '第1波は押し波から（仮定）'],
    refs: [
    ...
    description:
      ...
      '周期20分・3波・押し波から・揺れ90秒は設定値（仮定）です。',
  },
  {
    id: 'meio',
    name: '明応型地震',
  ...
  // ---- 内閣府 南海トラフ巨大地震 --------------------------------------------
  {
    ...
    description:
      ...
      '藤沢市の最大震度は6弱（内閣府。県の令和7年被害想定では5弱〜6弱）。周期20分・3波・押し波から・揺れ180秒は設定値（仮定）です。潮位は内閣府の条件（年間最高潮位を参考にした満潮位）ではなく T.P.+0.85m で計算します。',
  },
  ...
  // ---- 説明用の例（公的な想定ではない） --------------------------------------
  {
    ...
    assumptions: ['到達60分（実際は約3時間後。短縮した設定値）', ASSUME_FAR_PERIOD, ASSUME_WAVES, '揺れを感じない（震度0）として揺れの演出なし'],
