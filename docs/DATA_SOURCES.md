...
| 重ねるハザードマップ 津波浸水想定 | 公式の浸水想定の重ね表示・凡例。人物の評価（公式の浸水想定区域の判定。サイト内のミラー `public/tiles/hazard-tsunami` を優先: 4 章） | `https://disaportaldata.gsi.go.jp/raster/04_tsunami_newlegend_data/{z}/{x}/{y}.png` | 公共データ利用規約（第1.0版）。商用・非商用とも可、出典の記載が必要 | 「ハザードマップポータルサイト」（津波浸水想定：神奈川県）。人物の評価の判定に使う場面では「…を加工して作成」 |
...
### 人物の評価への利用（サイト内のミラー `public/tiles/hazard-tsunami`）

- 同じ統合版のタイル（ズーム 15）を `scripts/prefetch-dem.mjs --only hazard` でサイト内に複製し（計算範囲を覆う 48 枚のうち浸水想定のある 17 枚。
  残りの 31 枚は配信元も 404 なので `public/tiles/manifest.json` に 0 と記録。2026-09-24 取得）、画素の色を下の凡例の階級に読み替えて使う
  （`src/data/officialHazard.ts`・`officialHazardLoad.ts`。ミラーに無いタイルは配信元から取得し、どちらからも取得できない範囲は「不明」）。
- 使いみち: 人物の「最寄りの高台」の候補から公式の浸水想定区域（と周囲 30 m）を除き、人物の出発地点・避難先が公式の想定で何 m の区域かを示す（docs/MODEL.md 5.3・5.6）。
- 複製・加工は公共データ利用規約（第1.0版）で出典を記載すれば認められている（上記）。色を読み替えて判定に使うので、画面では編集・加工の記載例にならい、
  データの作成者を併記した `OFFICIAL_ZONE_CREDIT`「「ハザードマップポータルサイト」（津波浸水想定：神奈川県）を加工して作成」を表示する（「情報」タブ・人物の詳細）。
- 地図に重ねて表示する「公式ハザードマップ」レイヤーは、ミラーではなく配信元のタイル（ズーム 17 まで）を読む。接続できないときは、色が無いだけに見えないよう
  「レイヤー」タブと HUD にその旨を示す。
...
- 気象庁「津波警報・注意報、津波情報、津波予報について」 https://www.jma.go.jp/jma/kishou/know/jishin/joho/tsunamiinfo.html （`src/data/warnings.ts`）
  - 表「津波予報の発表条件」には「発表される場合」が 2 行ある（2026-09-24 に原文で確認）。
    「0.2ｍ未満の海面変動が予想されたとき」の内容「高いところでも0.2ｍ未満の海面変動のため被害の心配はなく、特段の防災対応の必要がない旨を発表します。」を
    津波予報（若干の海面変動）の区分の内容（`WARNING_INFO.forecast.action`）とし、「津波注意報解除後も海面変動が継続するとき」の内容は、
    注意報が出ていない本アプリの例には当てはまらないので、別の場合の参考（`WARNING_INFO.forecast.note`）として警報カードの詳細にだけ示す。
...
- 公式の津波浸水想定のタイルをサイト内に複製して人物の評価に使うようにしたので、4 章に使いみちと出典の表示（加工して作成）を追記。
- 気象庁「津波警報・注意報、津波情報、津波予報について」の表「津波予報の発表条件」を原文で確認し、津波予報（若干の海面変動）の区分の内容を
  「0.2ｍ未満の海面変動が予想されたとき」の行の文にした（それまでは「津波注意報解除後も海面変動が継続するとき」の行の文を入れていた。
  この行は別の場合の参考として `WARNING_INFO.forecast.note` に分けた。9 章）。
